    ...
    def __init__(self, funct, exist=None):
        ...
        def read_ini_data():
            if self.funct=='read':
                ...
                path=os.path.join(dir, 'Documents', '' )
                directory=askopenfile(mode='r', initialdir=path, defaultextension=".plt", 
            ...
            for key in keys:
                try:
                    ...
                    pass
            config.read(self.ini_path)
            keys=["x_style","x_scale","x_auto","y_style","y_scale","y_auto"]
            for key in keys:
                try:
            ...
            if funct=='load':# Place Data In List
                Plot_Data.clear()
                config.read(self.ini_path)
                for key, value in config.items('LINE_DATA'):
                    try:
        ...
        def write_ini_data():
            if self.funct=='write':
                ...
                dir=os.path.expanduser( '~' )
                path=os.path.join(dir, 'Documents', '' )
                directory=asksaveasfile(initialdir=path,
                    defaultextension=".plt",filetypes=[("PLT","*.plt"),("TXT","*.txt")])     
                if directory is None:return
                else: self.ini_path=directory.name
            config = configparser.ConfigParser()
            config.read(self.ini_path)
...
def open_plot_file():
    dir=os.path.expanduser( '~' )
    directory=os.path.join(dir, 'Documents', '' )
    path=askopenfile(mode='r', initialdir=directory, defaultextension=".plt", 
        filetypes=[("PLT","*.plt"),("TXT","*.txt")])
    if path is None:return
    os.system("notepad "+path.name)
def load_plot_file():
    Init_Graph('load')# Write Properties To File
    # Get Number Of Plots For Loops
    # Below Code Written For Only 5 Plots On 1 Graph
    #############################################
    t_num,p_num=0,0
    num_plts,start_index,end_index=[],[],[]
    for i in arange(0,len(Plot_Data)):
        t_num=Plot_Data[i][0]
        if t_num>p_num:
            p_num=t_num
            num_plts.append(p_num)
            start_index.append(i)
    if len(num_plts)==1:end_index.append(len(Plot_Data))
    elif len(num_plts)==2:
        end_index.append(start_index[1])        
        end_index.append(len(Plot_Data))        
    elif len(num_plts)==3:
        end_index.append(start_index[1])        
        end_index.append(start_index[2])        
        end_index.append(len(Plot_Data))        
    elif len(num_plts)==4:
        end_index.append(start_index[1])        
        end_index.append(start_index[2])        
        end_index.append(start_index[3])        
        end_index.append(len(Plot_Data))        
    elif len(num_plts)==5:
        end_index.append(start_index[1])        
        end_index.append(start_index[2])        
        end_index.append(start_index[3])        
        end_index.append(start_index[4])        
        end_index.append(len(Plot_Data))        
    #############################################
    if len(num_plts)>=1:
...
def about():
    messagebox.showinfo('About Graph-It Easy', 'Creator: Ross Waters\nEmail: RossWatersjr@gmail.com'\
        ...
        '\nRevision \ Date: 1.1.8 \ 02/19/2023'\
...
if __name__ == "__main__":
    ...
    file.add_command(label ='Open Plot File', command=lambda:open_plot_file())
    file.add_separator()
    file.add_command(label ='Save Plot File As', command=lambda:Init_Graph('save'))
    file.add_separator()
    file.add_command(label ='Load Plot File', command=lambda:load_plot_file())
    file.add_separator()
    ...
    configure_g.add_separator()
    configure_g.add_command(label ='Viewport Face Color', command=lambda:config_g('viewport color'))
    configure_g.add_separator()
    configure_g.add_command(label ='Viewport Major Grid Line Color', command=lambda:config_g('major color'))
    configure_g.add_separator()
    ...
    configure_g.add_cascade(label='Viewport Major Grid Line Style', menu=major)
    configure_g.add_separator()
    configure_g.add_command(label ='Viewport Minor Grid Line Color', command=lambda:config_g('minor color'))
    configure_g.add_separator()
    ...
    configure_g.add_cascade(label='Viewport Minor Grid Line Style', menu=minor)
    configure_g.add_separator()
    configure_g.add_command(label ='Show Minor Grid Lines', command=lambda:config_g('show minor'))
    configure_g.add_separator()
    configure_g.add_command(label ='Clear Viewport Data', command=lambda:config_g('clear'))
    configure_g.add_separator()
