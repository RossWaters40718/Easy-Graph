...
class Test():
    def log(exe_style):# Example
        ...
        if exe_style=='loop':set_title('X Scale = Log, Draw Style = Loop Update Using Lists')
        elif exe_style=='instant':set_title('X Scale = Log, Draw Style = Instant Update Using Lists')
        elif exe_style=='live':set_title('X Scale = Log, Draw Style = Live Update')
        ...
        x=np.logspace(1.3011,4.3011,num=100,endpoint=True,base=10)
        ...
        if exe_style=='loop': # @ 300 fps
            ...
            x_data=np.logspace(1.3011,4.3011,num=100,endpoint=True,base=10)
        ...
        line_color='yellow' # Set The Line Color
        ...
        if exe_style=='loop': # Loop Through Lists Data
            ...
            x_data=np.logspace(1.0,4.3011,num=100,endpoint=True,base=10)
            y_data=np.random.uniform(low=-10.0, high=10.0, size=(100,))
            # Generate Your X,Y Data Here And Create Seperate List For X Data And Y Data As They Arrive.
            # Each Time The Lists Gets Inserted Or Appended, The List Data Will Be Executed Inside The Loop.
            # This Should Be Used For Live Update Type Plots.
    ...
    def linear(exe_style):# Example
        clear_graph() # Always Call clear_graph() To Start Fresh.
        #////******* Place Test Code Here *******\\\\#
        if exe_style=='loop':set_title('X Scale = Linear, Draw Style = Loop Update Using Lists')
        elif exe_style=='instant':set_title('X Scale = Linear, Draw Style = Instant Update Using Lists')
        elif exe_style=='live':set_title('X Scale = Linear, Draw Style = Live Update')
        ...
        if exe_style=='loop':
            for i in np.arange(1,len(x_data)+1): # Start Should Always Be 1 With Stop At Length + 1
                ...
                ax.draw_artist(line)# Draw The Animated Artist (points) From The cached Renderer
                fig.canvas.blit(ax.bbox)# Renderer To The GUI Framework For Visualization
                newline=[plt_num,x_data[i-1],y_data[i-1],line_color,line_style,line_width]
                Plot_Data.insert(index,newline)
                index+=1
                fig.canvas.flush_events()# Flush Pending Events And Re-paint Canvas
                #time.sleep(0.01)
            ...
            for i in range(0, len(x_data)): # Data For Saving To File
                newline=[plt_num,x_data[i],y_data[i],line_color,line_style,line_width]
                Plot_Data.insert(index,newline)
                index+=1
        elif exe_style=='live':
            x_data=np.linspace(0.0,500.0, num=100)# 100 linear x values 0.0 to 500
        ...
        line_color='lime' # Set The Line Color
        line_style='dotted' # Set The Line Style
        line_width=2 # Set The Line Width
        # Place Your Lists Data Here
...
def about():
    messagebox.showinfo('About Graph-It Easy', 'Creator: Ross Waters\nEmail: RossWatersjr@gmail.com'\
        ...
        '\nRevision \ Date: 1.1.3 \ 02/04/2023'\
